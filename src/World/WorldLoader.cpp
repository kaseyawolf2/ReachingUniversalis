...
// Set at the start of Load(), cleared at the end.  Avoids threading
// the pointer through every file-scoped static helper.
static std::vector<LoadWarning>* s_warnings = nullptr;
...
static bool LoadNeeds(const std::string& path, WorldSchema& schema, std::string& err) {
    ...
    for (size_t i = 0; i < arr->size(); ++i) {
        ...
        if (def.satisfyBehavior != "consume" && def.satisfyBehavior != "sleep" && def.satisfyBehavior != "warmth") {
            PushWarning(LoadWarningLevel::Warning, "needs",
                    "[WorldLoader] WARNING: %s: need '%s' has unknown satisfy_behavior '%s' "
    ...
    // to change a default, update the constant there (single source of truth).
    {
        ...
        auto rangeCheck = [&](float val, const char* name) {
            if (val < 0.0f || val > 2.0f)
                PushWarning(LoadWarningLevel::Warning, "seasons",
                        "[WorldLoader] WARNING: %s: '%s' = %.3f is out of range [0.0, 2.0]\n",
                        path.c_str(), name, val);
        };
        ...
        if (!(st.mildCold < st.coldSeason))
            PushWarning(LoadWarningLevel::Warning, "seasons",
                    "[WorldLoader] WARNING: %s: mild_cold (%.3f) should be less than cold_season (%.3f)\n",
                    path.c_str(), st.mildCold, st.coldSeason);
        if (!(st.coldSeason < st.moderateCold))
            PushWarning(LoadWarningLevel::Warning, "seasons",
                    "[WorldLoader] WARNING: %s: cold_season (%.3f) should be less than moderate_cold (%.3f)\n",
                    path.c_str(), st.coldSeason, st.moderateCold);
        if (!(st.moderateCold < st.harshCold))
            PushWarning(LoadWarningLevel::Warning, "seasons",
                    "[WorldLoader] WARNING: %s: moderate_cold (%.3f) should be less than harsh_cold (%.3f)\n",
        ...
        if (!(st.mildCold < st.moderateCold))
            PushWarning(LoadWarningLevel::Warning, "seasons",
                    "[WorldLoader] WARNING: %s: cold threshold ordering violated: "
        ...
        if (!(st.lowProduction < st.harvestSeason))
            PushWarning(LoadWarningLevel::Warning, "seasons",
                    "[WorldLoader] WARNING: %s: production threshold ordering violated: "
        ...
        if (auto* pf = item->get("price_floors")) {
            if (auto* pfTbl = pf->as_table()) {
                for (auto& [key, val] : *pfTbl) {
                    ...
                    if (resId < 0) {
                        PushWarning(LoadWarningLevel::Warning, "seasons",
                                "[WorldLoader] WARNING: %s: price_floors references unknown resource '%s'\n",
...
static bool LoadEvents(const std::string& path, WorldSchema& schema, std::string& err) {
    ...
    for (size_t i = 0; i < arr->size(); ++i) {
        ...
        if      (effectType == "production_modifier") def.effectEnum = EventEffectType::ProductionModifier;
            ...
            PushWarning(LoadWarningLevel::Warning, "events",
                    "[WorldLoader] WARNING: %s: event '%s' has unknown effect_type '%s', defaulting to None\n",
...
static bool LoadGoals(const std::string& path, WorldSchema& schema, std::string& err) {
    ...
    for (size_t i = 0; i < arr->size(); ++i) {
        ...
        if (checkType == "balance_gte")         def.checkTypeEnum = GoalCheckType::BalanceGte;
            ...
            PushWarning(LoadWarningLevel::Warning, "goals",
                    "[WorldLoader] WARNING: %s: goal '%s' has unknown check_type '%s', defaulting to None\n",
        ...
        if (targetMode == "fixed")                  def.targetModeEnum = GoalTargetMode::Fixed;
            ...
            PushWarning(LoadWarningLevel::Warning, "goals",
                    "[WorldLoader] WARNING: %s: goal '%s' has unknown target_mode '%s', defaulting to Fixed\n",
        ...
        if (behaviourMod.empty() || behaviourMod == "none")
            ...
            PushWarning(LoadWarningLevel::Warning, "goals",
                    "[WorldLoader] WARNING: %s: goal '%s' has unknown behaviour_mod '%s', defaulting to None\n",
...
bool WorldLoader::Load(const std::string& worldDir,
    ...
    // Install the warning collector for this Load() call.
    // Scope guard ensures s_warnings is cleared on every exit path.
    s_warnings = warnings;
    if (s_warnings) s_warnings->clear();
    struct Guard { ~Guard() { s_warnings = nullptr; } } guard;
    ...
    if (!schema.events.empty()) {
        ...
        if (totalChance <= 0.f) {
            PushWarning(LoadWarningLevel::Warning, "events",
                    "[WorldLoader] WARNING: %s/events.toml: total event chance is zero "
    ...
    for (const auto& ev : schema.events) {
        ...
        case EventEffectType::ProductionModifier:
            if (ev.durationHours <= 0.f)
                PushWarning(LoadWarningLevel::Warning, "events",
                        "[WorldLoader] WARNING: %s/events.toml: event '%s': production_modifier effect has "
        ...
        case EventEffectType::StockpileDestroy:
            if (ev.targetResource.empty() && ev.destroyResource.empty() && ev.destroyResourceNames.empty())
                PushWarning(LoadWarningLevel::Warning, "events",
                        "[WorldLoader] WARNING: %s/events.toml: event '%s': stockpile_destroy effect has no "
        ...
        case EventEffectType::RoadBlock:
            if (ev.roadBlockDuration <= 0.f)
                PushWarning(LoadWarningLevel::Warning, "events",
                        "[WorldLoader] WARNING: %s/events.toml: event '%s': road_block effect has "
        ...
        case EventEffectType::Earthquake:
            if (ev.roadBlockDuration <= 0.f)
                PushWarning(LoadWarningLevel::Warning, "events",
                        "[WorldLoader] WARNING: %s/events.toml: event '%s': earthquake effect has "
                        "road_block_duration <= 0 (will fall back to hardcoded default)\n",
                        worldDir.c_str(), ev.name.c_str());
        ...
        case EventEffectType::MoraleBoost:
            if (ev.moraleImpact == 0.f && ev.treasuryChange == 0.f)
                PushWarning(LoadWarningLevel::Warning, "events",
                        "[WorldLoader] WARNING: %s/events.toml: event '%s': morale_boost effect has "
        ...
        if (ev.chance < 0.f)
            PushWarning(LoadWarningLevel::Warning, "events",
                    "[WorldLoader] WARNING: %s/events.toml: event '%s': has negative chance value %f\n",
    ...
    for (const auto& goal : schema.goals) {
        ...
        case GoalCheckType::BalanceGte:
            if (goal.targetValue <= 0.f && goal.targetModeEnum == GoalTargetMode::Fixed)
                PushWarning(LoadWarningLevel::Warning, "goals",
                        "[WorldLoader] WARNING: %s/goals.toml: goal '%s': balance_gte check has "
        ...
        case GoalCheckType::AgeGte:
            if (goal.targetValue <= 0.f && goal.targetModeEnum == GoalTargetMode::Fixed)
                PushWarning(LoadWarningLevel::Warning, "goals",
                        "[WorldLoader] WARNING: %s/goals.toml: goal '%s': age_gte check has "
                        "target_value <= 0 with fixed mode (goal is trivially complete)\n",
                        worldDir.c_str(), goal.name.c_str());
            break;
        case GoalCheckType::HasProfession:
            if (goal.targetProfessionId == INVALID_ID)
                PushWarning(LoadWarningLevel::Warning, "goals",
                        "[WorldLoader] WARNING: %s/goals.toml: goal '%s': has_profession check has no "
        ...
        if (goal.weight <= 0.f)
            PushWarning(LoadWarningLevel::Warning, "goals",
                    "[WorldLoader] WARNING: %s/goals.toml: goal '%s': has weight <= 0 "
    ...
    if (!schema.goals.empty()) {
        ...
        if (totalGoalWeight <= 0.f) {
            PushWarning(LoadWarningLevel::Warning, "goals",
                    "[WorldLoader] WARNING: %s/goals.toml: total goal weight is zero "
    ...
    PushWarning(LoadWarningLevel::Info, "summary",
            "[WorldLoader] Loaded '%s': %zu needs, %zu resources, "
    ...
    if (s_warnings && !s_warnings->empty()) {
        int warnCount = 0;
        for (const auto& w : *s_warnings)
