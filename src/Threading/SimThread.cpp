...
void SimThread::WriteSnapshot() {
    ...
    std::string snapSeasonName = "Spring";
    {
        auto stmv = m_registry.view<TimeManager>();
        if (stmv.begin() != stmv.end()) {
            snapSeasonId = stmv.get<TimeManager>(*stmv.begin()).CurrentSeason(m_schema);
            if (snapSeasonId >= 0 && snapSeasonId < (int)m_schema.seasons.size())
                snapSeasonName = m_schema.seasons[snapSeasonId].displayName;
        }
    }
    ...
    m_registry.view<Position, Settlement>().each(
        ...
        settlements.push_back({
            ...
            food, water, wood, spop, s.popCap, snapSeasonId, snapSeasonName, specialty,
    ...
    float  curHeatMult  = 0.f;
    {
        ...
        if (tmv3.begin() != tmv3.end()) {
            curSeasonId  = tmv3.get<TimeManager>(*tmv3.begin()).CurrentSeason(m_schema);
            curSeasonMod = SeasonProductionModifier(curSeasonId, m_schema);
            curHeatMult  = SeasonHeatDrainMult(curSeasonId, m_schema);
        }
    }
...

    {
        ...
        if (tmv.begin() != tmv.end()) {
            ...
            paused      = tm.paused;
            seasonId    = tm.CurrentSeason(m_schema);
            if (seasonId >= 0 && seasonId < (int)m_schema.seasons.size()) {
                seasonName          = m_schema.seasons[seasonId].displayName;
                seasonProductionMod = m_schema.seasons[seasonId].productionMod;
                seasonHeatDrainMod  = m_schema.seasons[seasonId].heatDrainMod;
            }
            temperature = AmbientTemperature(seasonId, m_schema, hourOfDay);
        }
    }
