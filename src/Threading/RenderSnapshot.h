...
struct RenderSnapshot {
    ...
    struct AgentEntry {
        ...
        // Parallel skillNames vector holds display names for HUD rendering.
    ...
    struct StockpilePanel {
        ...
        std::vector<int>              masterCount;
        std::vector<int>              journeymanCount;
        std::vector<std::string>      skillNames;  // parallel display names
    ...
    std::vector<float>       playerSkillLevels;  // empty = no Skills component
    std::vector<std::string> playerSkillNames;   // display names parallel to levels
