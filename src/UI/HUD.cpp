...
void HUD::Draw(const RenderSnapshot& snap, const Camera2D& camera, bool roadBuildMode) {
...

    {
        ...
        seasonProdMod        = snap.seasonProdMod;
        seasonHeatMod        = snap.seasonHeatMod;
    ...
    // ---- Time panel (top-right) ----
    {
        ...
        // Season color derived from properties: warm = yellow, cold = skyblue,
        // harvest = orange, moderate = green
        Color seasonColor = (seasonHeatMod >= 0.8f) ? SKYBLUE :
                            (seasonProdMod >= 1.1f) ? ORANGE  :
                            (seasonHeatMod <= 0.01f && seasonProdMod >= 0.9f) ? YELLOW : GREEN;
...
void HUD::DrawWorldStatus(const RenderSnapshot& snap) const {
    ...
    float heatMod;
    {
        std::lock_guard<std::mutex> lock(snap.mutex);
        ws      = snap.worldStatus;
        heatMod = snap.seasonHeatMod;
    }
    if (ws.empty()) return;

    bool showWood = (heatMod >= 0.15f);  // show wood prominently in cold seasons
    ...
    for (int i = 0; i < count; ++i) {
        ...
        Color woodFill = (s.wood < 20.f && (heatMod >= 0.8f)) ? RED :
...
void HUD::DrawFacilityTooltip(const RenderSnapshot& snap, const Camera2D& cam) const {
    ...
    float seasonProdModLocal;
    {
        std::lock_guard<std::mutex> lock(snap.mutex);
        seasonProdModLocal = snap.seasonProdMod;
    }
    ...
    float seasonMult = seasonProdModLocal;
