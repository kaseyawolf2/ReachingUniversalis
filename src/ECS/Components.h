...
// Social interaction cooldowns, visit state, and mood/milestone flags.
// Mandatory component emplaced on all NPCs at spawn; use get<> in the main
// NPC loop, try_get<> only when accessing other entities.
//
// Fields are grouped into three nested structs for clarity:
//   cooldowns  — timers that gate how often a social action can fire
//   visit      — family-visit tracking (timer + target settlement)
//   mood       — milestone flags, emotional buffs, and gratitude tracking
struct SocialBehavior {
    // -- Interaction cooldowns (all start at 0 = ready) -----------------------
    struct InteractionCooldowns {
        float gossipCooldown        = 0.f;   // game-hours until NPC can gossip prices again
        float gossipNudgeTimer      = 0.f;   // game-seconds remaining for gossip drift animation
        float chatTimer             = 0.f;   // game-seconds remaining in evening chat stop (0 = free to move)
        float greetCooldown         = 0.f;   // real-seconds until NPC can greet a neighbour again
        float thankCooldown         = 0.f;   // real-seconds until NPC can thank player again
        float teachCooldown         = 0.f;   // real-seconds until NPC can teach/learn again
        float moodContagionCooldown = 0.f;   // game-seconds until NPC can receive mood boost again
        float comfortCooldown       = 0.f;   // real-seconds until NPC can comfort a grieving neighbour again
        float begTimer              = 0.f;   // game-hours until NPC can beg from a friend again
    } cooldowns;
