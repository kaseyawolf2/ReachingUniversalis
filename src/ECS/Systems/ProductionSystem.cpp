...
void ProductionSystem::Update(entt::registry& registry, float realDt, const WorldSchema& schema) {
    ...
    SeasonID seasonId = tm.CurrentSeason(schema);
    float seasonMod   = SeasonProductionModifier(seasonId, schema);

    // ---- Food spoilage ----
    // Summer doubles the spoilage rate (heat); winter halves it (cold preserves food).
    // Look up season name from schema for behaviour that was previously enum-based.
    std::string seasonName;
    if (seasonId >= 0 && seasonId < (int)schema.seasons.size())
        seasonName = schema.seasons[seasonId].name;
    float spoilageMult = (seasonName == "Summer") ? 2.0f :
                         (seasonName == "Winter") ? 0.5f : 1.0f;
