...
void ScheduleSystem::Update(entt::registry& registry, float realDt, const WorldSchema& schema) {
    ...
    for (auto entity : view) {
        ...
        if (registry.all_of<ChildTag>(entity)) {
            ...
            if (ageC && ageC->days >= 15.f) {
                ...
                if (auto* sk = registry.try_get<Skills>(entity))
                    sk->Advance(primary, 0.15f, schema);
                ...
                if (!logv2.empty()) {
                    ...
                    if (const auto* sk = registry.try_get<Skills>(entity)) {
                        ...
                        if (bestId >= 0) {
                            float best = sk->Get(bestId);
                            std::string skName = (bestId < (int)schema.skills.size())
                                ? schema.skills[bestId].displayName : "Skill";
                            char skBuf[64];
                            std::snprintf(skBuf, sizeof(skBuf), " — best skill: %s %.0f%%", skName.c_str(), best * 100.f);
                            msg += skBuf;
                        }
                    }
        ...
        int seasonSleepAdj  = (seasonHeatDrain >= schema.seasonThresholds.harshCold) ? -2 : 0;  // negative = earlier sleep in harsh cold
        int seasonWakeAdj   = (seasonHeatDrain >= schema.seasonThresholds.harshCold) ?  1 : 0;  // positive = later wake in harsh cold
        int seasonWorkEndAdj = (seasonHeatDrain >= schema.seasonThresholds.harshCold) ? -2 : 0; // negative = earlier end in harsh cold
        ...
        if (state.behavior == AgentBehavior::Working &&
            ...
            if (const auto* skills = registry.try_get<Skills>(entity)) {
                ...
                if (bestId >= 0 && skills->Get(bestId) > 0.25f) {
                    hasAptitude = true;
                    if (bestId < (int)schema.skills.size())
                        aptitude = schema.skills[bestId].forResource;
                }
            }
            ...
            if (workFac != entt::null) {
                ...
                if (workDist > WORK_ARRIVE * WORK_ARRIVE) {
                    ...
                    if (hourChanged) {
                        int newProf = schema.ProfessionForResource(facType);
                        if (auto* prof = registry.try_get<Profession>(entity)) {
                            if (prof->type != newProf && schema.ProfessionProduces(newProf)) {
                                static std::mt19937 s_profRng{ std::random_device{}() };
                                if (schema.ProfessionProduces(prof->type)
                                    && s_profRng() % 2 == 0) {
                                    auto logV = registry.view<EventLog>();
                                    if (!logV.empty()) {
                                        std::string who = "NPC";
                                ...
                                if (schema.ProfessionProduces(prof->prevType) &&
                                ...
                                if (auto* sk = registry.try_get<Skills>(entity)) {
                                    SkillID oldSkId = SkillForProfession(prof->type, schema);
                                    float oldSkill = (oldSkId != INVALID_ID) ? sk->Get(oldSkId) : 0.f;
                                    if (oldSkill >= 0.5f) {
                                        SkillID newSkId = SkillForProfession(newProf, schema);
                                        if (newSkId != INVALID_ID)
                                            sk->Set(newSkId, std::min(0.5f, sk->Get(newSkId) + 0.05f));
                                    }
                                }
                                prof->prevType = prof->type;
                                prof->type = newProf;
                    ...
                    if (hourChanged) {
                        ...
                        if (mySkills && myProf && myRel2) {
                            ...
                            SkillID myProfSkId = SkillForProfession(myProf->type, schema);
                            float myProfSkill = (myProfSkId != INVALID_ID) ? mySkills->Get(myProfSkId) : 0.f;
                            if (myProfSkill >= 0.7f) {
                                registry.view<AgentState, Position, HomeSettlement, Skills, Profession>(
                                    ...
                                    SkillID oSkId = SkillForProfession(oPr.type, schema);
                                    float otherSkill = (oSkId != INVALID_ID) ? oSk.Get(oSkId) : 0.f;
                    ...
                    if (hourChanged) {
                        ...
                        if (mySkillsT && myProfT && myRelT
                            && schema.ProfessionProduces(myProfT->type)) {
                            SkillID mySkIdT = SkillForProfession(myProfT->type, schema);
                            float mySkillT = (mySkIdT != INVALID_ID) ? mySkillsT->Get(mySkIdT) : 0.f;
                            if (mySkillT >= 0.5f) {
                                registry.view<AgentState, Position, HomeSettlement, Skills, Profession>(
                                    entt::exclude<Hauler, PlayerTag, ChildTag>).each(
                                    [&](auto other, const AgentState& oState, const Position& oPos,
                                    ...
                                    if (!schema.ProfessionProduces(oPr.type)) return;
                                    float odx = oPos.x - fpos.x, ody = oPos.y - fpos.y;
                                    if (odx*odx + ody*ody > WORK_ARRIVE * WORK_ARRIVE) return;
                                    SkillID oSkIdT = SkillForProfession(oPr.type, schema);
                                    float otherSkillT = (oSkIdT != INVALID_ID) ? oSk.Get(oSkIdT) : 0.f;
                    ...
                    if (hourChanged) {
                        ...
                        if (myProfR && myRelR
                            && schema.ProfessionProduces(myProfR->type)) {
                            registry.view<AgentState, Position, HomeSettlement, Profession>(
                                ...
                                if (auto* sbA = registry.try_get<SocialBehavior>(entity))
                                    sbA->cooldowns.reconcileGlow = glowDuration;
                                if (auto* sbB = registry.try_get<SocialBehavior>(other))
                                    sbB->cooldowns.reconcileGlow = glowDuration;
                    ...
                    if (hourChanged) {
                        ...
                        if (mySongProf && schema.ProfessionProduces(mySongProf->type)) {
                            ...
                            int songChance = (seasonProdMod > schema.seasonThresholds.harvestSeason) ? 15 : 30;
                            float songAffinityGain = (seasonHeatDrain >= schema.seasonThresholds.harshCold) ? 0.02f : 0.01f;
                                ...
                                if (!logVS.empty()) {
                                    ...
                                    if (home.settlement != entt::null && registry.valid(home.settlement))
                                        if (const auto* s = registry.try_get<Settlement>(home.settlement))
                                            where = s->name;
                                    std::string songMsg;
                                    if (seasonProdMod > schema.seasonThresholds.harvestSeason)
                                        songMsg = who + " leads a harvest shanty at " + where + ".";
                                    else if (seasonHeatDrain >= schema.seasonThresholds.harshCold)
                    ...
                    if (auto* skills = registry.try_get<Skills>(entity)) {
                        ...
                        if (const auto* prof = registry.try_get<Profession>(entity))
                            if (prof->type == schema.ProfessionForResource(facType))
                                gainMult = 1.1f;
                        ...
                        float before = skills->ForResource(facType, schema);
                        skills->Advance(facType, SKILL_GAIN_PER_GAME_HOUR * gameHoursDt * gainMult, schema);
                        float after = skills->ForResource(facType, schema);
                            ...
                            if (before < threshold && after >= threshold) {
                                ...
                                if (s_milestones.insert(key).second) {
                                    ...
                                    if (idx == 0 || idx == 1) {
                                        ...
                                        sbCeleb.cooldowns.skillCelebrateTimer = 0.5f; // 0.5 game-hours = ~30 real-seconds at 1×
                                    }
