...
entt::entity AgentDecisionSystem::FindMigrationTarget(entt::registry& registry,
    ...
    if (profession && profession->type >= 0 && profession->type < (int)schema.professions.size()) {
        int res = schema.professions[profession->type].producesResource;
        if (res != INVALID_ID) {
            profAffinityType = res;
            hasProfAffinity = true;
        }
    }
...
void AgentDecisionSystem::Update(entt::registry& registry, float realDt, const WorldSchema& schema) {
    // Helper: get the profession's produced resource for skill lookup
    auto profRes = [&](int profId) -> int {
                ...
                [&](auto e, Skills& sk, const Profession& prof, const Age& age, const HomeSettlement& hs) {
                    ...
                    bool loyal = (prof.prevType == prof.type
                                  || (prof.prevType >= 0 && prof.prevType < (int)schema.professions.size()
                                      && schema.professions[prof.prevType].isIdle));
                    ...
                    if (const auto* sbGrowth = registry.try_get<SocialBehavior>(e)) {
                        if (sbGrowth->mood.bankruptSurvivor) {
                            growth += 0.0002f;
                    ...
                    if (myProfFlag.any() && hs.settlement != entt::null) {
                        ...
                        if (myRel) {
                            ...
                            for (const auto& [target, aff] : myRel->affinity) {
                                ...
                                const auto* tSk = registry.try_get<Skills>(target);
                                if (!tSk) continue;
                                float rivalSkill = tSk->Get(profSkId);
                                if (rivalSkill >= 0.8f) {
                    ...
                    {
                        float postActiveSkill = (profSkId != INVALID_ID) ? sk.Get(profSkId) : 0.f;
                        const char* profName = schema.ProfessionLabel(prof.type);
                        if (loyal && profSkId != INVALID_ID && preActiveSkill < 0.7f && postActiveSkill >= 0.7f
                    ...
                    {
                        float postActiveSkill2 = (profSkId != INVALID_ID) ? sk.Get(profSkId) : 0.f;
                        const char* recSkill = (profSkId >= 0 && profSkId < (int)schema.skills.size())
                            ? schema.skills[profSkId].displayName.c_str() : nullptr;
                    ...
                    {
                        float postSkill3 = (profSkId != INVALID_ID) ? sk.Get(profSkId) : 0.f;
                        const char* prideProfName = schema.ProfessionLabel(prof.type);
                        if (profSkId != INVALID_ID && preActiveSkill < 0.8f && postSkill3 >= 0.8f) {
                            ...
                            if (hs.settlement != entt::null) {
                                registry.view<Profession, Skills, HomeSettlement, Relations>(
                                    ...
                                    [&](auto jealousE, const Profession& jPr, const Skills& jSk,
                                        ...
                                        int jProfSkId = SkillForProfession(jPr.type, schema);
                                        float jSkill = (jProfSkId >= 0) ? jSk.Get(jProfSkId) : 0.f;
                    ...
                    if (!registry.all_of<Hauler>(e)) {
                        if (auto* sbMaster = registry.try_get<SocialBehavior>(e)) {
                            if (!sbMaster->mood.masterSettled && sk.AnyAbove(MASTER_THRESHOLD))
                                sbMaster->mood.masterSettled = true;
                        }
                    }
                    ...
                    if (masterGrowthApplied && !logV2.empty()) {
                        if (s_teachRng() % 10 == 0) {
                            ...
                            if (hs.settlement != entt::null && registry.valid(hs.settlement))
                                if (const auto* s = registry.try_get<Settlement>(hs.settlement))
                                    settlName = s->name;
                            std::string skillName2 = (profSkId >= 0 && profSkId < (int)schema.skills.size())
                                ? schema.skills[profSkId].displayName : "craft";
    ...
    // ---- Mourning procession: 3+ grieving NPCs at same settlement gather to honour the fallen elder ----
    {
        ...
        if (tm.day != s_lastProcessionDay) {
            ...
            registry.view<Skills, HomeSettlement, AgentState>(
                entt::exclude<Hauler, PlayerTag, BanditTag>).each(
                [&](auto e, const Skills& sk, const HomeSettlement& hs, const AgentState&) {
            ...
            for (auto& [settlE, gi] : griefBySettl) {
                ...
                registry.view<Skills, HomeSettlement, AgentState>(
                    entt::exclude<Hauler, PlayerTag, BanditTag>).each(
                    [&](auto e, const Skills& sk, const HomeSettlement& hs, AgentState& st) {
    ...
    // ---- Mentor-apprentice: elders (age > 60) mentor children (age 12-14) at the same settlement ----
    {
        ...
        if (tm.day != s_lastMentorDay) {
            ...
            struct ElderInfo { entt::entity e; int prof; };
            ...
            registry.view<Age, Profession, HomeSettlement>(
                ...
                [&](auto e, const Age& age, const Profession& prof, const HomeSettlement& hs) {
                    if (age.days > 60.f && hs.settlement != entt::null
                        && schema.ProfessionProduces(prof.type))
            ...
            for (auto child : childView) {
                ...
                const auto& childName = childView.get<Name>(child);
                {
                    ...
                    if (hs.settlement == entt::null) return;
                    if (!schema.ProfessionProduces(prof.type)) return;
                    ...
                    for (const auto& elder : it->second) {
                        ...
                        if (!logV.empty() && s_mentorRng() % 5 == 0) {
                            ...
                            if (const auto* nm = registry.try_get<Name>(elder.e))
                                elderName = nm->value;
                            SkillID mentSkId = SkillForProfession(prof.type, schema);
                            std::string skillNameM = (mentSkId >= 0 && mentSkId < (int)schema.skills.size())
                                ? schema.skills[mentSkId].displayName : "craft";
                            std::string settlName = "settlement";
                            if (registry.valid(hs.settlement))
                                if (const auto* s = registry.try_get<Settlement>(hs.settlement))
                                    settlName = s->name;
    ...
    {
        auto logV = registry.view<EventLog>();
        registry.view<Money, SocialBehavior, HomeSettlement, Name>(
            entt::exclude<PlayerTag, Hauler, BanditTag>).each(
            [&](auto e, const Money& m, SocialBehavior& sbW,
                const HomeSettlement& hs, const Name& name) {
            if (sbW.mood.wealthCelebrated) return;
            if (m.balance < 500.f) return;
            sbW.mood.wealthCelebrated = true;
            if (!logV.empty()) {
    ...
    registry.view<HomeSettlement, GriefState>(
        entt::exclude<Hauler, PlayerTag, BanditTag>).each(
        [&](auto e, const HomeSettlement& hs, const GriefState& gs) {
        if (gs.griefTimer > 0.f && hs.settlement != entt::null && registry.valid(hs.settlement))
    ...
    for (auto entity : view) {
        ...
        if (socialBeh.visit.timer > 0.f) {
            float gameMinDt = dt * GAME_MINS_PER_REAL_SEC;
            socialBeh.visit.timer -= gameMinDt;
            if (socialBeh.visit.timer <= 0.f) {
                // Visit over — return home
                socialBeh.visit.timer  = 0.f;
                socialBeh.visit.target = entt::null;
                if (home.settlement != entt::null && registry.valid(home.settlement)) {
                    const auto& homePos = registry.get<Position>(home.settlement);
                    MoveToward(vel, pos, homePos.x, homePos.y, speed);
                }
            } else if (socialBeh.visit.target != entt::null && registry.valid(socialBeh.visit.target)) {
                const auto& tgtPos = registry.get<Position>(socialBeh.visit.target);
        ...
        if (state.behavior == AgentBehavior::Celebrating) {
            ...
            if (skillCelebration && (static_cast<uint32_t>(entity) % 4 == static_cast<uint32_t>(s_frameCounter) % 4)) {
                ...
                if (myRel) {
                    registry.view<AgentState, Position>(
                        entt::exclude<Hauler, PlayerTag, BanditTag>).each(
        ...
        if (state.behavior == AgentBehavior::Migrating) {
            ...
            if (InRange(pos.x, pos.y, destPos.x, destPos.y, SETTLE_RANGE)) {
                ...
                socialBeh.mood.homesickTimer = 0.f;
                state.behavior        = AgentBehavior::Idle;
                ...
                if (home.settlement != entt::null && registry.valid(home.settlement)) {
                    ...
                    if (bestFac != entt::null) {
                        ...
                        if (auto* prof = registry.try_get<Profession>(entity)) {
                            ...
                            if (oldType != newType
                                && schema.ProfessionProduces(oldType)
                                && schema.ProfessionProduces(newType)) {
                                if (auto* sk = registry.try_get<Skills>(entity)) {
                                    int oldRes = profRes(oldType);
                                    int newRes = profRes(newType);
                                    // Halve old skill
                                    float oldVal = sk->ForResource(oldRes, schema);
                                    sk->Advance(oldRes, -(oldVal * 0.5f), schema);
                                    // Boost new skill by 10% (capped at 1.0 by Advance)
                                    sk->Advance(newRes, 0.1f, schema);
                                }
                            }
                        }
                    }
                ...
                // Log arrival with profession
                {
                    ...
                    if (!lv.empty()) {
                        ...
                        if (!tmv2.empty()) {
                            ...
                            if (prof && schema.ProfessionProduces(prof->type)) {
                                who += " (";
                                who += schema.ProfessionLabel(prof->type);
                                who += ")";
                            }
                ...
                if (socialBeh.mood.masterSettled) {
        ...
        if (home.prevSettlement != entt::null && registry.valid(home.prevSettlement)
            ...
            socialBeh.mood.homesickTimer += ghDtHS;
            if (socialBeh.mood.homesickTimer > 72.f && timer.lastSatisfaction < 0.4f) {
        ...
        if (timer.stockpileEmpty >= effectiveMigrateThreshold) {
            ...
            if (dest != entt::null) {
                ...
                if (skills) {
                    const char* masterSkill = nullptr;
                    std::string masterSkillName;
                    int bestId = skills->BestSkillId();
                    if (bestId >= 0 && skills->Get(bestId) >= 0.9f) {
                        if (bestId < (int)schema.skills.size())
                            masterSkillName = schema.skills[bestId].displayName;
                        else
                            masterSkillName = "unknown";
                        masterSkill = masterSkillName.c_str();
                    }
                    if (masterSkill) {
                        auto lv3 = registry.view<EventLog>();
                        if (!lv3.empty()) {
                ...
                if (socialBeh.mood.masterSettled && home.settlement != entt::null && registry.valid(home.settlement)) {
                    if (auto* settl = registry.try_get<Settlement>(home.settlement)) {
                ...
                if (const auto* rel = registry.try_get<Relations>(entity)) {
                    ...
                    if (bestFriend != entt::null) {
                        ...
                        auto* fDepTimer   = registry.try_get<DeprivationTimer>(bestFriend);
                        float fSatisfaction = fDepTimer ? fDepTimer->lastSatisfaction : 0.5f;
                            ...
                            if (const auto* fRel = registry.try_get<Relations>(bestFriend)) {
                                ...
                                for (const auto& [candidate, aff] : fRel->affinity) {
                                    ...
                                    if (aff > bestThirdAff) {
                                        ...
                                        auto* cDepTimer   = registry.try_get<DeprivationTimer>(candidate);
                                        float cSat = cDepTimer ? cDepTimer->lastSatisfaction : 0.5f;
                                ...
                                if (thirdNpc != entt::null) {
                                    ...
                                    auto* cSocial2 = registry.try_get<SocialBehavior>(thirdNpc);
                                    if (cState2) {
                                        cState2->behavior = AgentBehavior::Migrating;
                                        cState2->target   = dest;
                                    }
                                    {
                                        auto* cDepTimer2 = registry.try_get<DeprivationTimer>(thirdNpc);
                                        if (cDepTimer2)
                                            cDepTimer2->stockpileEmpty = 0.f;
                                    }
                                }
                            }
        ...
        if (critIdx == -1) {
            ...
            if (socialBeh.cooldowns.chatTimer > 0.f) {
                socialBeh.cooldowns.chatTimer -= dt;
                vel.vx = vel.vy = 0.f;
                continue;
            }
            ...
            if (socialBeh.cooldowns.gossipNudgeTimer > 0.f)
                socialBeh.cooldowns.gossipNudgeTimer = std::max(0.f, socialBeh.cooldowns.gossipNudgeTimer - dt);
            if (currentHour >= 20 && currentHour < 22 && socialBeh.cooldowns.gossipNudgeTimer <= 0.f
                && home.settlement != entt::null && registry.valid(home.settlement)) {
                ...
                registry.view<AgentState, Position, HomeSettlement>(
                    ...
                    socialBeh.cooldowns.gossipNudgeTimer = 3.f;  // cooldown: 3 game-seconds
                    found = true;
                });
            }

            // ---- Greeting: idle NPCs occasionally greet a nearby idle neighbour ----
            if (socialBeh.cooldowns.greetCooldown > 0.f)
                socialBeh.cooldowns.greetCooldown = std::max(0.f, socialBeh.cooldowns.greetCooldown - dt);
            if (!isGrieving && socialBeh.cooldowns.greetCooldown <= 0.f &&
                home.settlement != entt::null && registry.valid(home.settlement)) {
                ...
                registry.view<AgentState, Position, HomeSettlement, Name>(
                    ...
                    socialBeh.cooldowns.greetCooldown  = 2.f;
                    if (oSocial) oSocial->cooldowns.greetCooldown = 2.f;
                    // Check if this is a gratitude greeting (greeter's lastHelper == other)
                    bool isGratitude = (socialBeh.mood.lastHelper != entt::null && socialBeh.mood.lastHelper == other);
                    ...
                    }
                    {
                        ...
                        if (lv.begin() != lv.end()) {
                            ...
                            if (isGratitude) {
                                ...
                                socialBeh.mood.lastHelper = entt::null;  // gratitude expressed, clear
                    ...
                    if (oSocial && playerEntity != entt::null &&
                        socialBeh.mood.lastHelper == playerEntity &&
                        oSocial->mood.lastHelper != playerEntity) {
                        oSocial->mood.lastHelper = playerEntity;
                        auto lv2 = registry.view<EventLog>();
                        if (lv2.begin() != lv2.end()) {
            ...
            if (socialBeh.cooldowns.comfortCooldown > 0.f)
                socialBeh.cooldowns.comfortCooldown = std::max(0.f, socialBeh.cooldowns.comfortCooldown - realDt);
            if (socialBeh.cooldowns.comfortCooldown <= 0.f && griefSt.griefTimer <= 0.f
                ...
                if (myRel) {
                    ...
                    registry.view<AgentState, Position, Name>(
                        entt::exclude<Hauler, PlayerTag, BanditTag>).each(
                        [&](auto other, const AgentState& oState, const Position& oPos,
                            ...
                            if (other == entity) return;
                            if (oState.behavior != AgentBehavior::Idle) return;
                            if (!oGrief || oGrief->griefTimer <= 0.f) return;
                            ...
                            socialBeh.cooldowns.comfortCooldown = 180.f; // 180 real-seconds
            ...
            if (isGrieving && static_cast<uint32_t>(entity) % 4 == static_cast<uint32_t>(s_frameCounter) % 4) {
                ...
                registry.view<HomeSettlement, Relations>(
                    entt::exclude<Hauler, PlayerTag, BanditTag>).each(
                    [&](auto other, const HomeSettlement& oHome, Relations& oRel) {
                        auto* oGrief = registry.try_get<GriefState>(other);
                        if (other == entity) return;
                        if (!oGrief || oGrief->griefTimer <= 0.f) return;
                        if (oHome.settlement != home.settlement) return;
            ...
            if (socialBeh.cooldowns.thankCooldown > 0.f)
                socialBeh.cooldowns.thankCooldown = std::max(0.f, socialBeh.cooldowns.thankCooldown - realDt);
            if (playerEntity != entt::null && socialBeh.cooldowns.thankCooldown <= 0.f
                && static_cast<uint32_t>(entity) % 4 == static_cast<uint32_t>(s_frameCounter) % 4) {
                ...
                if (const auto* rep = registry.try_get<Reputation>(entity)) {
                    if (rep->score > 0.3f) {
                        ...
                        if (tdx*tdx + tdy*tdy <= THANK_RADIUS * THANK_RADIUS) {
                            socialBeh.cooldowns.thankCooldown = 60.f;  // 60 real-seconds cooldown
                            auto lv = registry.view<EventLog>();
                            if (!lv.empty()) {
                                const auto* myName = registry.try_get<Name>(entity);
                                std::string msg = (myName ? myName->value : "An NPC") +
                ...
                if (avgN > 0.8f) {
                    ...
                    if (wdx * wdx + wdy * wdy <= WAVE_RADIUS * WAVE_RADIUS) {
                        ...
                        if (s_waveDist(s_waveRng) < 0.01f * realDt) {
                            socialBeh.cooldowns.thankCooldown = 60.f;
                            auto lv = registry.view<EventLog>();
                            if (lv.begin() != lv.end()) {
                                const auto* myName = registry.try_get<Name>(entity);
                                std::string msg = (myName ? myName->value : "An NPC") +
            ...
            if (socialBeh.cooldowns.teachCooldown > 0.f)
                socialBeh.cooldowns.teachCooldown = std::max(0.f, socialBeh.cooldowns.teachCooldown - realDt);
            if (socialBeh.cooldowns.teachCooldown <= 0.f
                && static_cast<uint32_t>(entity) % 4 == static_cast<uint32_t>(s_frameCounter) % 4) {
                if (auto* mySkills = registry.try_get<Skills>(entity)) {
                    ...
                    registry.view<AgentState, Position, HomeSettlement, Skills, Name>(
                        ...
                        if (oState.behavior != AgentBehavior::Idle) return;
                        auto* oSocial = registry.try_get<SocialBehavior>(other);
                        if (!oSocial || oSocial->cooldowns.teachCooldown > 0.f) return;
                        ...
                        for (const auto& skillDef : schema.skills) {
                            ...
                            if (tVal >= TEACH_MIN && lVal < LEARN_MAX) {
                                ...
                                // Cooldowns
                                socialBeh.cooldowns.teachCooldown  = 120.f;
                                if (oSocial) oSocial->cooldowns.teachCooldown = 120.f;
                                // Mentor bond: learner remembers teacher for gratitude greeting
                                if (oSocial) oSocial->mood.lastHelper = entity;
                                // Log
                                auto lv = registry.view<EventLog>();
                                if (lv.begin() != lv.end()) {
                                    const auto* myName = registry.try_get<Name>(entity);
            ...
            if (socialBeh.cooldowns.moodContagionCooldown > 0.f)
                socialBeh.cooldowns.moodContagionCooldown = std::max(0.f, socialBeh.cooldowns.moodContagionCooldown - realDt);
            {
                float avgNeeds = 0.f;
                ...
                if (avgNeeds < 0.4f && socialBeh.cooldowns.moodContagionCooldown <= 0.f
                    && static_cast<uint32_t>(entity) % 4 == static_cast<uint32_t>(s_frameCounter) % 4) {
                    registry.view<Needs, Position, AgentState, Name>(
                        ...
                        [&](auto other, const Needs& oNeeds, const Position& oPos,
                            ...
                            socialBeh.cooldowns.moodContagionCooldown = 120.f; // 120 game-seconds
                            // Log
                            auto lv = registry.view<EventLog>();
                            if (lv.begin() != lv.end()) {
                                const auto* myName = registry.try_get<Name>(entity);
            ...
            if (!isGrieving) if (auto* ft = registry.try_get<FamilyTag>(entity); ft) {
                ...
                if (s_visitChance(s_visitRng) < 0.05f * visitGameHoursDt) {
                    ...
                    if (visitSettl != entt::null) {
                        socialBeh.visit.timer  = 30.f;  // 30 game-minutes
                        socialBeh.visit.target = visitSettl;
            ...
            if (currentHour >= 18 && currentHour < 21 &&
                ...
                if (dist2 > GATHER_ARRIVE * GATHER_ARRIVE) {
                    ...
                    registry.view<AgentState, Position, HomeSettlement>(
                        ...
                        auto* oSocial = registry.try_get<SocialBehavior>(other);
                        auto* oGrief  = registry.try_get<GriefState>(other);
                        if (!oSocial || oSocial->cooldowns.chatTimer > 0.f) return;  // already chatting or missing component
                        float cdx = oPos.x - pos.x, cdy = oPos.y - pos.y;
                        ...
                        socialBeh.cooldowns.chatTimer  = dur;
                        if (oSocial) oSocial->cooldowns.chatTimer = dur;
                        ...
                        // Expert gratitude: novice gains extra affinity toward expert of same profession
                        {
                            ...
                            if (profA && profB && skA && skB && profA->type == profB->type
                                && schema.ProfessionProduces(profA->type)) {
                                SkillID chatSkId = SkillForProfession(profA->type, schema);
                                float sA = (chatSkId != INVALID_ID) ? skA->Get(chatSkId) : 0.f;
                                float sB = (chatSkId != INVALID_ID) ? skB->Get(chatSkId) : 0.f;
                        ...
                        if (oSocial && socialBeh.mood.bankruptSurvivor != oSocial->mood.bankruptSurvivor && s_chatRng() % 10 == 0) {
                            entt::entity survivor = socialBeh.mood.bankruptSurvivor ? entity : other;
                            entt::entity listener2 = socialBeh.mood.bankruptSurvivor ? other : entity;
    ...
    registry.view<Position, HomeSettlement, DeprivationTimer>(
        ...
        [&](auto e, const Position& p, const HomeSettlement& hs, DeprivationTimer& tmr) {
            ...
            auto* tmrSocial = registry.try_get<SocialBehavior>(e);
            if (tmrSocial && tmrSocial->cooldowns.gossipCooldown > 0.f)
                tmrSocial->cooldowns.gossipCooldown = std::max(0.f, tmrSocial->cooldowns.gossipCooldown - gameHoursDt);
            if (hs.settlement != entt::null && registry.valid(hs.settlement))
    ...
    for (std::size_t i = 0; i < gossipAgents.size(); ++i) {
        ...
        auto* tmrASocial = registry.try_get<SocialBehavior>(A.entity);
        if (!tmrASocial || tmrASocial->cooldowns.gossipCooldown > 0.f) continue;
        ...
        for (std::size_t j = i + 1; j < gossipAgents.size(); ++j) {
            ...
            auto* tmrBSocial = registry.try_get<SocialBehavior>(B.entity);
            auto* mktB = registry.try_get<Market>(B.homeSettl);
            if (!mktB) continue;

            bool bWasReady = (tmrBSocial && tmrBSocial->cooldowns.gossipCooldown <= 0.f);
            ...
            if (bWasReady) {
                ...
                tmrBSocial->cooldowns.gossipCooldown = GOSSIP_COOLDOWN;
            }
            tmrASocial->cooldowns.gossipCooldown = GOSSIP_COOLDOWN;
            ...
            // 10% chance per gossip encounter to spread illness from sick to healthy NPC.
            {
                ...
                auto trySpread = [&](entt::entity sick, entt::entity healthy) {
                    auto* dtSickPes = registry.try_get<PersonalEventState>(sick);
                    auto* dtHealthyPes = registry.try_get<PersonalEventState>(healthy);
                    if (!dtSickPes || !dtHealthyPes) return;
                    if (dtSickPes->illnessTimer <= 0.f || dtHealthyPes->illnessTimer > 0.f) return;
                    if (s_illDist(s_illRng) > CONTAGION_CHANCE) return;
                    dtHealthyPes->illnessTimer   = CONTAGION_ILLNESS_DUR;
                    dtHealthyPes->illnessNeedIdx = dtSickPes->illnessNeedIdx;
                    auto lv = registry.view<EventLog>();
                    if (lv.begin() != lv.end()) {
    ...
    registry.view<Position, Needs, Money, HomeSettlement, DeprivationTimer>(
        ...
        [&](auto e, const Position& p, const Needs& n, const Money& m,
            ...
            auto* tmrCharity = registry.try_get<CharityState>(e);
            if (tmrCharity) { tmrCharity->charityTimer = std::max(0.f, tmrCharity->charityTimer - gameHoursDt); }
            if (tmrCharity) { tmrCharity->helpedTimer  = std::max(0.f, tmrCharity->helpedTimer  - gameHoursDt); }
            if (hs.settlement == entt::null || !registry.valid(hs.settlement)) return;
            float hunger = ((int)NeedType::Hunger < (int)n.list.size())
                         ? n.list[(int)NeedType::Hunger].value : 1.f;
            charityAgents.push_back(CharityEntry{
                e, p.x, p.y, hunger, m.balance, hs.settlement,
                /*canHelp=*/  (hunger >= HUNGER_HELPER && m.balance >= MONEY_HELPER_MIN
                               && tmrCharity && tmrCharity->charityTimer <= 0.f),
    ...
    for (auto& helper : charityAgents) {
      ...
      for (int pass = 0; pass < 2 && !helped; ++pass) {
        for (auto& starving : charityAgents) {
            ...
            if (auto* helperTmrCheck = registry.try_get<CharityState>(helper.entity))
            ...
            auto* starvingCharity = registry.try_get<CharityState>(starving.entity);
            auto* starvingSocial  = registry.try_get<SocialBehavior>(starving.entity);
            ...
            // Set gratitude walk: move toward helper for 30–60 real-seconds.
            {
                ...
                if (starvingCharity) {
                    starvingCharity->helpedTimer     = 1.f;   // 1 game-hour display window
                    starvingCharity->gratitudeTarget = helper.entity;
                }
                if (starvingSocial) starvingSocial->mood.lastHelper = helper.entity;
                static std::uniform_real_distribution<float> s_gratDist(30.f, 60.f);
                static std::mt19937 s_gratRng{ std::random_device{}() };
                if (starvingCharity) starvingCharity->gratitudeTimer  = s_gratDist(s_gratRng);
            }

            // Set helper cooldown
            auto* helperCharity = registry.try_get<CharityState>(helper.entity);
            if (helperCharity) helperCharity->charityTimer = CHARITY_COOLDOWN;
    ...
    // ============================================================
    {
        ...
        if (!orphans.empty()) {
            ...
            registry.view<Position, HomeSettlement, DeprivationTimer>(
                ...
                auto* atmCharity = registry.try_get<CharityState>(ae);
                if (ahs.settlement == entt::null || !registry.valid(ahs.settlement)) return;
                if (!atmCharity || atmCharity->charityTimer > 0.f) return;
                ...
                atmCharity->charityTimer = ADOPT_COOLDOWN;
    ...
    // ============================================================
    {
        ...
        registry.view<Relations, Money, HomeSettlement, DeprivationTimer, Name>(
            ...
            auto* giverCharity = registry.try_get<CharityState>(giver);
            if (!giverCharity || giverCharity->charityTimer > 0.f) return;
            ...
            giverCharity->charityTimer = GIFT_COOLDOWN;
    ...
    registry.view<Position, Velocity, MoveSpeed, Needs, AgentState,
        ...
        {
            auto* banditSt = registry.try_get<BanditState>(e);
            auto* theftRec = registry.try_get<TheftRecord>(e);
            ...
            if (home.settlement != entt::null && registry.valid(home.settlement)) {
                ...
                if (banditSt) banditSt->banditPovertyTimer = 0.f;
                return;
            }
            ...
            if (!isBanditNow && money.balance >= RESETTLE_COST) {
                ...
                if (bestSettl != entt::null) {
                    ...
                    if (theftRec) theftRec->theftCount = 0;
                    if (banditSt) banditSt->banditPovertyTimer = 0.f;
                    if (banditLog) {
            ...
            if (!isBanditNow) {
                if (money.balance < BANDIT_POVERTY_THRESH && banditSt) {
                    banditSt->banditPovertyTimer += gameHoursDt;
                    if (banditSt->banditPovertyTimer >= BANDIT_PROMOTE_HOURS) {
                    ...
                    banditSt->banditPovertyTimer = 0.f;
                }
            }
            ...
            if (lurkRoad != entt::null) {
                ...
                std::string oldGangName = banditSt ? banditSt->gangName : "";
                if (banditsPerRoad[lurkRoad] >= 2) {
                    // Try to copy an existing gang name from another bandit at this road
                    std::string existingGang;
                    registry.view<Position, BanditTag>(
                        entt::exclude<Hauler, PlayerTag>).each(
                        [&](auto other, const Position& op) {
                            if (other == e || !existingGang.empty()) return;
                            auto* oBandit = registry.try_get<BanditState>(other);
                            if (!oBandit || oBandit->gangName.empty()) return;
                            // Check if this bandit is also targeting the same road
                            auto [otherRoad, otherMid] = findNearestRoad(op.x, op.y);
                            if (otherRoad == lurkRoad) existingGang = oBandit->gangName;
                        });
                    if (banditSt) {
                      if (!existingGang.empty()) {
                        banditSt->gangName = existingGang;
                      } else {
                        ...
                        if (!nA.empty() && !nB.empty())
                            banditSt->gangName = "The " + nA + "-" + nB + " Wolves";
                        else
                            banditSt->gangName = "Road Wolves";
                      }
                    }
                    // Log when a bandit first joins a gang
                    if (banditSt && oldGangName.empty() && !banditSt->gangName.empty() && banditLog) {
            ...
            if (intercepted) {
                ...
                registry.view<Position, Velocity, MoveSpeed>(
                    ...
                    [&](auto npcE, const Position& npos, Velocity& nvel,
                        ...
                        if (auto* ntmrBandit = registry.try_get<BanditState>(npcE))
                            ntmrBandit->panicTimer = 2.f;
                    });
            }
