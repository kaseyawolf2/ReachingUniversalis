...
entt::entity AgentDecisionSystem::FindMigrationTarget(entt::registry& registry,
    ...
    registry.view<Road>().each([&](const Road& road) {
        ...
        auto tmv = registry.view<TimeManager>();
        if (!tmv.empty()) {
            SeasonID seasonId = tmv.get<TimeManager>(*tmv.begin()).CurrentSeason();
            float heatMult = GetSeasonHeatDrainMod(seasonId, schema);
            if (heatMult > 0.f) {
        ...
        // are less attractive, making NPCs more inclined to stay put.
        {
            ...
            if (!tmv2.empty()) {
                SeasonID sid = tmv2.get<TimeManager>(*tmv2.begin()).CurrentSeason();
                // Cold seasons (high heat drain) make migration less attractive
                if (GetSeasonHeatDrainMod(sid, schema) >= 0.8f) total *= 0.8f;
            }
        }
