...
entt::entity AgentDecisionSystem::FindMigrationTarget(entt::registry& registry,
    ...
    if (profession && m_schema) {
        ProfessionID pt = profession->type;
        if (pt >= 0 && pt < (int)m_schema->professions.size()) {
            int pRes = m_schema->professions[pt].producesResource;
            if (pRes != INVALID_ID) {
                profAffinityType = pRes;
...
void AgentDecisionSystem::Update(entt::registry& registry, float realDt, const WorldSchema& schema) {
    ...
    // Helper lambdas for data-driven profession lookups
        ...
        if (tm.day != s_lastSkillGrowthDay) {
            ...
            registry.view<Skills, Profession, HomeSettlement>(
                ...
                [&](const Skills& sk, const Profession& prof, const HomeSettlement& hs) {
                    ...
                    if (profSkillVal(sk, prof.type) >= MASTER_THRESHOLD) {
                        int flag = profBit(prof.type);
            ...
            registry.view<Skills, Profession, HomeSettlement>(
                ...
                [&](const Skills& sk, const Profession& prof, const HomeSettlement& hs) {
                    if (hs.settlement == entt::null) return;
                    if (profSkillVal(sk, prof.type) >= 0.8f) {
                        int flag = profBit(prof.type);
            ...
            // Key: settlement → vector of (entity, profBitMask) for elders with skill >= 0.8
            struct ElderInfo2 { entt::entity e; int profBitMask; };
            std::unordered_map<entt::entity, std::vector<ElderInfo2>> skilledElders;
            registry.view<Skills, Profession, Age, HomeSettlement>(
                ...
                [&](auto elder, const Skills& sk, const Profession& prof, const Age& age, const HomeSettlement& hs) {
                    ...
                    int flag = profBit(prof.type);
                    float skill = profSkillVal(sk, prof.type);
                    if (flag && skill >= 0.8f)
            ...
            registry.view<Skills, Profession, Age, HomeSettlement>(
                ...
                [&](auto e, Skills& sk, const Profession& prof, const Age& age, const HomeSettlement& hs) {
                    ...
                    int pFlag = profBit(prof.type);
                    bool hasMaster = false;
                    if (pFlag && hs.settlement != entt::null) {
                        auto mit = masterFlags.find(hs.settlement);
                        hasMaster = (mit != masterFlags.end() && (mit->second & pFlag));
                    }
                    // Don't boost masters themselves
                    float growth = SKILL_GROWTH;
                    // Loyalty bonus: NPCs who never changed profession grow faster
                    bool loyal = (prof.prevType == prof.type || profIsIdle(prof.prevType));
                    ...
                    if (pFlag && hs.settlement != entt::null) {
                        float activeSkill = profSkillVal(sk, prof.type);
                        if (activeSkill >= 0.7f) {
                            auto ait = activeMentoring.find(hs.settlement);
                            if (ait != activeMentoring.end() && (ait->second & pFlag)) {
                                growth *= 1.1f;
                                rivalryActive = true;
                            }
                        }
                        ...
                        if (eit != skilledElders.end()) {
                            ...
                            if (rel) {
                                ...
                                for (const auto& info : eit->second) {
                                    if (!(info.profBitMask & pFlag)) continue;
                    ...
                    if (pFlag && hs.settlement != entt::null) {
                        float activeSkill2 = profSkillVal(sk, prof.type);
                        if (activeSkill2 < 0.5f) {
                            auto xit = expertFlags.find(hs.settlement);
                            if (xit != expertFlags.end() && (xit->second & pFlag)) {
                                growth += 0.0004f;
                                ...
                                if (novRel) {
                                    for (auto& [target, aff] : novRel->affinity) {
                                        ...
                                        if (!tSk) continue;
                                        float tSkill = profSkillVal(*tSk, prof.type);
                                        if (tSkill >= 0.8f) {
                    ...
                    if (pFlag && hs.settlement != entt::null) {
                        ...
                        if (myRel) {
                            ...
                            for (const auto& [target, aff] : myRel->affinity) {
                                ...
                                const auto* tSk = registry.try_get<Skills>(target);
                                if (!tSk) continue;
                                float rivalSkill = profSkillVal(*tSk, prof.type);
                                if (rivalSkill >= 0.8f) {
                    ...
                    float preActiveSkill = profSkillVal(sk, prof.type);
                    {
                        int pRes2 = profRes(prof.type);
                        if (pRes2 != INVALID_ID) {
                            float curVal = sk.ForResource(pRes2);
                            if (hasMaster && curVal < MASTER_THRESHOLD)
                                growth = MASTER_SKILL_GROWTH + (loyal ? LOYALTY_BONUS : 0.f);
                            sk.Advance(pRes2, growth);
                        }
                    }
                    // Loyalty streak: log when a loyal NPC crosses the 0.7 skill threshold
                    {
                        float postActiveSkill = profSkillVal(sk, prof.type);
                        std::string profName2 = (prof.type >= 0 && prof.type < (int)schema.professions.size())
                            ? schema.professions[prof.type].displayName : "";
                        if (loyal && !profName2.empty() && preActiveSkill < 0.7f && postActiveSkill >= 0.7f
                            ...
                            if (hs.settlement != entt::null && registry.valid(hs.settlement))
                                if (const auto* s = registry.try_get<Settlement>(hs.settlement))
                                    where = s->name;
                            logV2.get<EventLog>(*logV2.begin()).Push(tm.day, (int)tm.hourOfDay,
                                who + " is a dedicated " + profName2 + " at " + where + ".");
                        }
                    }
                    // Skill recovery celebration: log when active skill rises back above 0.5
                    {
                        float postActiveSkill2 = profSkillVal(sk, prof.type);
                        std::string recSkill = (prof.type >= 0 && prof.type < (int)schema.professions.size())
                            ? schema.professions[prof.type].displayName : "";
                        if (!recSkill.empty() && preActiveSkill < 0.5f && postActiveSkill2 >= 0.5f) {
                    ...
                    {
                        float postSkill3 = profSkillVal(sk, prof.type);
                        std::string prideProfName = (prof.type >= 0 && prof.type < (int)schema.professions.size())
                            ? schema.professions[prof.type].displayName : "";
                        if (!prideProfName.empty() && preActiveSkill < 0.8f && postSkill3 >= 0.8f) {
                            ...
                            if (!logV2.empty() && s_teachRng() % 3 == 0) {
                                ...
                                std::snprintf(buf, sizeof(buf), "%s proudly declares mastery of %s at %s",
                                              who.c_str(), prideProfName.c_str(), where.c_str());
                                logV2.get<EventLog>(*logV2.begin()).Push(tm.day, (int)tm.hourOfDay, buf);
                            }
                            ...
                            if (hs.settlement != entt::null) {
                                registry.view<Profession, Skills, HomeSettlement, Relations>(
                                    ...
                                    [&](auto jealousE, const Profession& jPr, const Skills& jSk,
                                        ...
                                        float jSkill = profSkillVal(jSk, jPr.type);
                    ...
                    if (hasMaster && growth > SKILL_GROWTH && !logV2.empty()) {
                        if (s_teachRng() % 10 == 0) {
                            ...
                            if (hs.settlement != entt::null && registry.valid(hs.settlement))
                                if (const auto* s = registry.try_get<Settlement>(hs.settlement))
                                    settlName = s->name;
                            std::string skillName3 = (prof.type >= 0 && prof.type < (int)schema.professions.size())
                                ? schema.professions[prof.type].displayName : "craft";
                            std::string msg = "A master inspires " +
                                std::string(nm ? nm->value : "an NPC") +
                                "'s " + skillName3 + " at " + settlName + ".";
    ...
    // ---- Mentor-apprentice: elders (age > 60) mentor children (age 12-14) at the same settlement ----
    {
        ...
        if (tm.day != s_lastMentorDay) {
            ...
            // Build per-settlement elder profession set: settlement → vector of (entity, profID)
            struct ElderInfo { entt::entity e; ProfessionID prof; };
            ...
            for (auto child : childView) {
                ...
                const auto& childName = childView.get<Name>(child);
                {
                    ...
                    if (hs.settlement == entt::null) return;
                    if (profIsIdle(prof.type)) return;
                    ...
                    for (const auto& elder : it->second) {
                        ...
                        {
                            int mentorRes = profRes(prof.type);
                            if (mentorRes != INVALID_ID)
                                sk.Advance(mentorRes, MENTOR_SKILL_GROWTH);
                        }
                        // Log at 1-in-5 frequency
                        if (!logV.empty() && s_mentorRng() % 5 == 0) {
                            ...
                            if (const auto* nm = registry.try_get<Name>(elder.e))
                                elderName = nm->value;
                            std::string skillName = (prof.type >= 0 && prof.type < (int)schema.professions.size())
                                ? schema.professions[prof.type].displayName : "craft";
                            std::string settlName = "settlement";
                            if (registry.valid(hs.settlement))
                                if (const auto* s = registry.try_get<Settlement>(hs.settlement))
                ...
                if (home.settlement != entt::null && registry.valid(home.settlement)) {
                    ...
                    if (bestFac != entt::null) {
                        ...
                        if (auto* prof = registry.try_get<Profession>(entity)) {
                            ProfessionID oldType = prof->type;
                            ProfessionID newType = schema.FindProfessionForResource(pf.output);
                            if (newType != INVALID_ID) prof->type = newType;

                            // Skill adjustment on profession change: halve old, boost new by 10%
                            if (oldType != newType && !profIsIdle(oldType)
                                && !profIsIdle(newType)) {
                                if (auto* sk = registry.try_get<Skills>(entity)) {
                                    int oldRes2 = profRes(oldType);
                                    int newRes2 = profRes(newType);
                                    // Halve old skill
                                    if (oldRes2 != INVALID_ID) {
                                        float oldVal = sk->ForResource(oldRes2);
                                        sk->Advance(oldRes2, -(oldVal * 0.5f));
                                    }
                                    // Boost new skill by 10% (capped at 1.0 by Advance)
                                    if (newRes2 != INVALID_ID)
                                        sk->Advance(newRes2, 0.1f);
                                }
                            }
                        }
                    }
                ...
                // Log arrival with profession
                {
                    ...
                    if (!lv.empty()) {
                        ...
                        if (!tmv2.empty()) {
                            ...
                            if (prof && !profIsIdle(prof->type)) {
                                who += " (";
                                if (prof->type >= 0 && prof->type < (int)schema.professions.size())
                                    who += schema.professions[prof->type].displayName;
                                who += ")";
                            }
                    ...
                    registry.view<AgentState, Position, HomeSettlement, DeprivationTimer>(
                        ...
                        // Expert gratitude: novice gains extra affinity toward expert of same profession
                        {
                            ...
                            if (profA && profB && skA && skB && profA->type == profB->type
                                && !profIsIdle(profA->type) && !profIsHauler(profA->type)) {
                                float sA = profSkillVal(*skA, profA->type);
                                float sB = profSkillVal(*skB, profB->type);
