...
- [x] **Hauler rival route competition** — In `TransportSystem.cpp`, tracks last hauler per route via
  ...
  affinity (floor 0.0). Logs undercutting message at 1-in-5 frequency.
...
- [x] **Drought solidarity** — In `RandomEventSystem.cpp`'s drought trigger (case 0), NPC pairs at the
  ...
  once per drought. Strengthens community bonds through shared hardship.
...
- [x] **Elder storytelling event** — In `AgentDecisionSystem.cpp`'s idle chat block, when elder
  ...
  toward elder boosted by +0.03. Logs "[Elder] tells [Listener] tales of the old days at [Settlement]."
...
- [x] **Trade gift between friends** — NPCs with `Relations::affinity ≥ 0.6` at the same settlement
  ...
  balance-to-balance. Logged to EventLog. In `AgentDecisionSystem.cpp` after orphan adoption block.
